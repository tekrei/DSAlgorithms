 ...
 *
 * There are numerous ways to implement this with the most common being to
 * determine a range
 * that the search key resides in and performing a binary search within that
 * range.
 * This takes O(log i) where i is the position of the search key in the list, if
 * the search key is in the list,
 * or the position where the search key should be, if the search key is not in
 * the list.
 *

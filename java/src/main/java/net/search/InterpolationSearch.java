 ...
 * and the key sought — it does not require numerical values for the keys, just
 * a total order on them.
 * The remaining search space is reduced to the part before or after the
 * estimated position.
 * The linear search uses equality only as it compares elements one-by-one from
 * the start, ignoring any sorting.
 *
...
public class InterpolationSearch extends Search {
    ...
    public int search(List<Integer> list, int searchKey) {
        ...
        while ((lowerBound <= upperBound)
                && (searchKey >= list.get(lowerBound) && (searchKey <= list.get(upperBound)))) {
